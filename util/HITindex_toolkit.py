...
def getJuncBed(bed, juncbam, readtype, readstrand):
	...
	if readstrand == 'none':
		...
		command = "intersectBed -abam " + juncbam + "_read.bam" + " -b " + bed + " -bed -wo -split | gzip > " + juncbam + "_exonjuncs.bed.gz"
		#command = "intersectBed -abam " + juncbam + " -b " + bed + " -bed -wo -split > " + juncbam_name + "_exonjuncs.bed"
		subprocess.call(command, shell=True)
	if readtype == 'single' and readstrand == 'r':
		# single bam file, read1=-S
		print("NOTE: fr-firststrand SE reads")
		command = "intersectBed -abam " + juncbam + "_read.bam" + " -b " + bed + " -bed -wo -S -split | gzip > " + juncbam + "_exonjuncs.bed.gz"
		subprocess.call(command, shell=True)
	if readtype == 'single' and readstrand == 'f':
		# single bam file, read1=-s
		print("NOTE: fr-secondstrand SE reads")
		command = "intersectBed -abam " + juncbam + "_read.bam" + " -b " + bed + " -bed -wo -s -split | gzip > " + juncbam + "_exonjuncs.bed.gz"
		subprocess.call(command, shell=True)
	if readtype == 'paired' and readstrand == 'rf':
		# two bams, read1=-S, read2=-s-
		print("NOTE: fr-firststrand PE reads")
		read1command = "intersectBed -abam " + juncbam + "_read1.bam -b " + bed + " -bed -wo -S -split > " + juncbam + "_read1.bed"
		read2command = "intersectBed -abam " + juncbam + "_read2.bam -b " + bed + " -bed -wo -s -split > " + juncbam + "_read2.bed"
		joincommand = "cat " + juncbam + "_read*.bed | gzip > " +juncbam +"_exonjuncs.bed.gz"  
		rmcommand = "rm -f " + juncbam + "_read*.bed"
		for command in (read1command, read2command, joincommand, rmcommand):
			subprocess.call(command, shell=True)
	if readtype == 'paired' and readstrand == 'fr':
		# two bams, read1=-s, read2=-S
...
def getExonReads(juncbam, overlap):
	# juncbam_name = juncbam[:-4]
	bedfile = gzip.open(juncbam + "_exonjuncs.bed.gz", 'rt')
...
def edge_flagging(HITdict):
	...
	if len(flaggingdata) == 0:
		print("No data was added to flaggingdata!")
